...
exports.twServer = function twServer(twdir, options) {
	...
	if (!options.dir.tws) {
		console.log(hue(`Can not find server directory`,9));
		return Promise.resolve();
	}
	if (options.server) {
		if (fs.existsSync(`${options.dir.tws}/tiddlywiki.info`)) {
			return serverTiddlyWiki(options.dir.tws, options.port)
			.catch((exitcode) => console.error(hue(`Error code: ${exitcode}`,9)));
		} else {
			console.log(hue(`${options.dir.tws} is not a 'server' edition TiddlyWiki`,9));
			return Promise.resolve();
		}
	}
}
