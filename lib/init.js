...
exports.twInit = function twInit(twdir, options) {
	...
	return initTiddlyWiki(options)
		.then(() => { if (options.dir.git) {
				console.log(hue(`# cloning to existing repository: ${options.dir.git}`,171));
		}})
		...
		.then(() => fs.promises.writeFile(`${options.dir.git}/.git/info/exclude`, ignoreTiddlers))
